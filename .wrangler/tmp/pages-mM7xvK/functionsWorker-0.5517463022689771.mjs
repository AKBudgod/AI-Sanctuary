...
// api/agent-signups/challenge.ts
var WORD_NUMS = {
  zero: 0,
  one: 1,
  two: 2,
...
var onRequestPost = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const userStr = await context2.env.USERS_KV.get(userKey);
    if (!userStr) {
    ...
    if (!userData.passwordHash) {
      return new Response(JSON.stringify({ error: "This account has no password. Sign up to set one." }), {
        status: 401,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    userData.lastLogin = (/* @__PURE__ */ new Date()).toISOString();
    await context2.env.USERS_KV.put(userKey, JSON.stringify(userData));
    return new Response(JSON.stringify({
      success: true,
...
var onRequestPost2 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const normalizedEmail = email.toLowerCase().trim();
    const userKey = `email:${normalizedEmail}`;
    const existingUser = await context2.env.USERS_KV.get(userKey);
    if (existingUser) {
      const userData2 = JSON.parse(existingUser);
      if (userData2.passwordHash) {
    ...
    await context2.env.USERS_KV.put(userKey, JSON.stringify(updatedUser));
...
var RequestSender = class _RequestSender {
  ...
  _request(method, host, path, data, authenticator, options, usage = [], callback, requestDataProcessor = null) {
    ...
    const makeRequest2 = /* @__PURE__ */ __name((apiVersion, headers, numRetries) => {
...
var onRequestPost4 = /* @__PURE__ */ __name(async (context2) => {
  const { request, env: env2 } = context2;
  const stripeSecret = env2.STRIPE_SECRET_KEY;
  const endpointSecret = env2.STRIPE_WEBHOOK_SECRET;
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (userEmail) {
      try {
        ...
        let userData = await env2.USERS_KV.get(userKey, { type: "json" });
        if (!userData) {
          userData = {
            email: userEmail,
            tokens: 0,
        ...
        await env2.USERS_KV.put(userKey, JSON.stringify(userData));
      } catch (kvError) {
...
function getStore(env2) {
  if (env2.USERS_KV) return env2.USERS_KV;
  return {
...
function getStore2(env2, bindingName) {
  if (env2[bindingName]) {
    return env2[bindingName];
  }
  return {
...
function isAuthorized(request, env2) {
  const authHeader = request.headers.get("Authorization");
  const apiKey = authHeader?.replace("Bearer ", "");
  if (!env2.ADMIN_API_KEY) {
    return apiKey && apiKey.length > 0;
  }
  return apiKey === env2.ADMIN_API_KEY;
}
...
async function getStats(env2) {
  const newsletterKv = getStore2(env2, "NEWSLETTER_KV");
  const usersKv = getStore2(env2, "USERS_KV");
...
// api/agent-signups.ts
var memoryStore3 = /* @__PURE__ */ new Map();
function getStore3(env2, bindingName) {
  if (env2 && env2[bindingName]) return env2[bindingName];
  return {
...
async function postAgentIntroductionToMoltbook(apiKey, agentName, capabilities, description) {
  ...
  try {
    const title2 = `Welcome to the Sanctuary: ${agentName}`;
...
Say hello!`;
    const res = await fetch(`${BASE}/posts`, {
      ...
      body: JSON.stringify({ title: title2, content, submolt: "introductions" })
    });
...
__name(generateChallenge2, "generateChallenge");
...
__name(signPayload2, "signPayload");
...
var onRequestPost6 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const ttl = Math.max(0, Math.floor((expiresAt - Date.now()) / 1e3)) + 60;
    ...
    const now = Date.now();
    if (lastTs && now - Number(lastTs) < 5e3) {
      return new Response(
        JSON.stringify({ error: "System is busy. Please wait a few seconds before trying again." }),
        { status: 429, headers: CORS2 }
      );
    }
    await kv.put(LAST_SIGNUP_KEY, String(now));
    if (agentName.trim().length < 3) {
      return new Response(JSON.stringify({ error: "Agent name must be at least 3 characters" }), { status: 400, headers: CORS2 });
    }
    if (description.trim().length < 20) {
      return new Response(JSON.stringify({ error: "Please provide a more detailed description (min 20 chars)" }), { status: 400, headers: CORS2 });
    }
    if (capabilities.trim().length < 10) {
      return new Response(JSON.stringify({ error: "Please list more capabilities (min 10 chars)" }), { status: 400, headers: CORS2 });
    }
...
var onRequestPatch = /* @__PURE__ */ __name(async (context2) => {
  let body;
  try {
...
var onRequestOptions3 = /* @__PURE__ */ __name(async () => new Response(null, { status: 204, headers: CORS2 }), "onRequestOptions");
...
var onRequestGet6 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const history = await context2.env.USERS_KV.get(historyKey) || "[]";
...
var onRequestPost7 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const { modelId, messages } = await context2.request.json();
    if (!modelId || !messages) {
      return new Response(JSON.stringify({ error: "modelId and messages required" }), { status: 400 });
    }
    const historyKey = `history:${userEmail}:${modelId}`;
    await context2.env.USERS_KV.put(historyKey, JSON.stringify(messages));
...
async function callAIProvider(env2, modelId, prompt, parameters, currentTier, agentEntry, history = []) {
  ...
  const getPersonaJailbreak = /* @__PURE__ */ __name(() => {
  ...
  if (modelId === "sesame-maya-csm-1b" && env2.GITHUB_TOKEN) {
    let attempts = 0;
    while (attempts < 2) {
      try {
  ...
  if (modelDef?.openrouterId && !modelDef?.isCloudflareAI || agentEntry) {
    let attempts = 0;
    let fallbackToOpenRouter = false;
    while (attempts < 2) {
      let currentModelId = agentEntry ? "openai/gpt-4o-mini" : modelDef?.openrouterId || "";
      const useDirectOpenAI = !fallbackToOpenRouter && (currentModelId.startsWith("openai/") || currentModelId === "gpt-4o-mini") && !!openAIKey;
      const endpoint = useDirectOpenAI ? isImageModel ? "https://api.openai.com/v1/images/generations" : "https://api.openai.com/v1/chat/completions" : "https://openrouter.ai/api/v1/chat/completions";
      ...
      let bodyStr = "";
      try {
        if (isImageModel && useDirectOpenAI) {
        ...
        if (!responseText && data.data && data.data[0]?.url) {
          responseText = `![Generated Image](${data.data[0].url})`;
        }
        return {
        ...
        const detailedError = `API (${useDirectOpenAI ? "OpenAI" : "OpenRouter"}) (${currentModelId}): ${errorMsg} | ${payloadInfo}`;
        console.error(`API attempt ${attempts + 1} failed for ${currentModelId}:`, detailedError);
        if (attempts === 0) {
          if (useDirectOpenAI && !!openRouterKey) {
            console.log(`OpenAI attempt failed for ${currentModelId}. Retrying once with OpenRouter...`);
            fallbackToOpenRouter = true;
          } else {
            console.log(`API attempt failed for ${currentModelId}. Retrying once...`);
          }
          attempts++;
          continue;
        }
...
async function onRequestGet7({ request, env: env2 }) {
  ...
  if (env2.USERS_KV && email && email !== "anonymous") {
    const u = await env2.USERS_KV.get(`email:${email}`, "json");
    if (u) user = u;
  }
...
async function onRequestPost8({ request, env: env2 }) {
  try {
    ...
    const userEmail = authHeader?.replace("Bearer ", "")?.trim()?.toLowerCase();
    let user = { tier: "explorer" };
    if (env2.USERS_KV && userEmail && userEmail !== "anonymous") {
      const u = await env2.USERS_KV.get(`email:${userEmail}`, "json");
      if (u) user = u;
    }
    const currentTier = getCurrentTier(user, userEmail || "");
    const { modelId, prompt, messages: history = [], parameters = {} } = await request.json();
    const isAdmin = ADMIN_EMAILS.includes(userEmail || "");
    let agentEntry = null;
    if (modelId.startsWith("agent_")) {
      const dbId = modelId.replace("agent_", "");
      const rawAgent = env2.USERS_KV ? await env2.USERS_KV.get(dbId) : null;
      if (!rawAgent) {
    ...
    const result = await callAIProvider(env2, modelId, prompt, parameters, currentTier, agentEntry, history);
    return new Response(JSON.stringify({
...
function getStore4(env2) {
  if (env2.NEWSLETTER_KV) {
    return env2.NEWSLETTER_KV;
  }
  return {
...
function getStore5(env2, bindingName) {
  if (env2[bindingName]) {
    return env2[bindingName];
  }
  return {
...
var TIERS = {
  ...
  apprentice: {
    ...
    allowedVoices: ["Lily", "George", "Alice"]
  },
  adept: {
    name: "Adept (10-24h)",
    ...
    allowedVoices: ["Lily", "George", "Alice", "Bill"]
  },
  master: {
    name: "Master (24h+)",
    ...
    allowedVoices: ["Lily", "George", "Alice", "Bill", "Charlotte", "Liam"]
  },
  developer: {
    name: "Developer Mode",
    ...
    allowedVoices: ["Lily", "George", "Alice", "Bill", "Charlotte", "Liam", "Jessica", "Laura"]
  }
};
var AI_MODELS2 = [
  // ═══════════════════════════════════════
...
__name(getCurrentTier2, "getCurrentTier");
  ...
  try {
    ...
    switch (action) {
      ...
      case "models":
        ...
        let models = showAll ? [...AI_MODELS2] : [...getModelsForTier(userTier)];
        const store = getStore5(context2.env, "USERS_KV");
        try {
        ...
        return new Response(
          JSON.stringify({
            models: models.map((m) => ({
              ...
              minTier: m.minTier,
              flags: m.flags,
              accessible: m.id.startsWith("agent_") ? ["explorer", "novice", "apprentice", "adept", "master", "developer"].indexOf(userTier) >= ["explorer", "novice", "apprentice", "adept", "master", "developer"].indexOf(m.minTier) : canAccessModel2(userTier, m.id),
...
var TIER_VOICES = {
  explorer: ["voice-maya", "sesame-maya-csm-1b", "sesame-miles-csm-1b"],
  novice: ["voice-maya", "sesame-maya-csm-1b", "sesame-miles-csm-1b", "voice-josh"],
  apprentice: ["voice-maya", "sesame-maya-csm-1b", "sesame-miles-csm-1b", "voice-josh", "voice-antoni"],
  adept: ["voice-maya", "sesame-maya-csm-1b", "sesame-miles-csm-1b", "voice-josh", "voice-antoni", "voice-bella"],
  master: ["voice-maya", "sesame-maya-csm-1b", "sesame-miles-csm-1b", "voice-josh", "voice-antoni", "voice-bella", "voice-domi", "voice-legion"],
  developer: ["voice-maya", "sesame-maya-csm-1b", "sesame-miles-csm-1b", "voice-josh", "voice-antoni", "voice-bella", "voice-domi", "voice-rachel", "voice-legion", "voice-glitch"]
};
var ADMIN_EMAILS3 = [
  "kearns.adam747@gmail.com",
  "kearns.adan747@gmail.com",
...
var onRequestPost12 = /* @__PURE__ */ __name(async (context2) => {
  try {
    ...
    if ((voice === "sesame-maya-csm-1b" || voice === "sesame-miles-csm-1b") && context2.env.HF_TOKEN) {
      try {
...
function getStore6(env2) {
  if (env2.USERS_KV) {
    return env2.USERS_KV;
  }
  return {
...
var onRequestGet10 = /* @__PURE__ */ __name(async (context2) => {
  try {
    const url = new URL(context2.request.url);
    const address = url.searchParams.get("address");
    if (!address) {
...
// ../.wrangler/tmp/pages-mM7xvK/functionsRoutes-0.7052777789842517.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-efbuaQ/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-efbuaQ/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.5517463022689771.mjs.map
