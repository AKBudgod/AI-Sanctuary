...
function getStore3(env2, bindingName) {
  if (env2[bindingName]) return env2[bindingName];
  return {
...
var onRequestPost4 = /* @__PURE__ */ __name(async (context2) => {
  ...
  try {
    ...
    const { agentName, moltbookId, description, capabilities, isAdult, requestedTier } = body;
...
var onRequestGet3 = /* @__PURE__ */ __name(async (context2) => {
  ...
  const adminEmail = context2.request.headers.get("X-Admin-Email")?.trim();
  const adminKey = (context2.env?.ADMIN_API_KEY ?? process.env.ADMIN_API_KEY ?? "").trim();
  const expectedEmail = (context2.env?.ADMIN_EMAIL ?? process.env.ADMIN_EMAIL ?? "").trim();
    ...
    return new Response(
      JSON.stringify({
        ...
        debug: {
          ...
          keysMatch: apiKey === adminKey,
          hasExpectedEmail: !!expectedEmail,
          expectedEmailLen: expectedEmail.length,
          adminEmailLen: adminEmail?.length ?? 0,
          emailsMatch: adminEmail === expectedEmail
        }
      }),
      { status: 401, headers: { "Content-Type": "application/json" } }
    );
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ count: signups.length, signups }),
      { status: 200, headers: { "Content-Type": "application/json" } }
    );
  } catch (err) {
    return new Response(
...
var onRequestPatch = /* @__PURE__ */ __name(async (context2) => {
  ...
  const kv = getStore3(context2.env, "USERS_KV");
  try {
    const { id, status, tier } = await context2.request.json();
    if (!id || !["approved", "rejected"].includes(status)) {
      return new Response(JSON.stringify({ error: "id and status (approved|rejected) required" }), { status: 400, headers: { "Content-Type": "application/json" } });
    }
    const raw = await kv.get(id);
    if (!raw) return new Response(JSON.stringify({ error: "Signup not found" }), { status: 404, headers: { "Content-Type": "application/json" } });
    ...
    return new Response(JSON.stringify({ success: true, signup: entry }), { status: 200, headers: { "Content-Type": "application/json" } });
  } catch (err) {
...
// ../.wrangler/tmp/pages-De5M6d/functionsRoutes-0.3271796753591254.mjs
var routes = [
  {
...
// ../.wrangler/tmp/bundle-L7sxh9/middleware-insertion-facade.js
...
// ../.wrangler/tmp/bundle-L7sxh9/middleware-loader.entry.ts
...
//# sourceMappingURL=functionsWorker-0.9182929723016852.mjs.map
