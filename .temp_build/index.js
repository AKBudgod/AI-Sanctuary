...
// api/purchase/create-checkout-session.ts
var onRequestPost = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    const { email, usd, tier } = await request.json();
  ...
  if (event.type === "checkout.session.completed") {
    ...
    if (userEmail) {
      try {
        ...
        let userData = await env.USERS_KV.get(userKey, { type: "json" });
        if (!userData) {
          userData = {
            email: userEmail,
            tokens: 0,
...
function getStore(env) {
  ...
  return {
    get: /* @__PURE__ */ __name(async (key) => memoryStore.get(key) || null, "get"),
...
var onRequestGet = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    let userKey = "";
    if (email) {
      userKey = `user:${email.toLowerCase()}`;
    } else if (address) {
      ...
      if (authHeader) {
        ...
        if (token.includes("@")) {
          userKey = `user:${token.toLowerCase()}`;
        } else {
...
function getStore2(env, bindingName) {
  if (env[bindingName]) {
    return env[bindingName];
  }
  return {
    get: /* @__PURE__ */ __name(async (key) => memoryStore2.get(key) || null, "get"),
    put: /* @__PURE__ */ __name(async (key, value) => {
    ...
    list: /* @__PURE__ */ __name(async (opts) => {
...
async function getStats(env) {
  const newsletterKv = getStore2(env, "NEWSLETTER_KV");
  const usersKv = getStore2(env, "USERS_KV");
...
var onRequestGet2 = /* @__PURE__ */ __name(async (context) => {
  ...
  try {
    const newsletterKv = getStore2(context.env, "NEWSLETTER_KV");
    const usersKv = getStore2(context.env, "USERS_KV");
    switch (action) {
      case "subscribers":
...
function getStore3(env, bindingName) {
  if (env[bindingName]) {
    return env[bindingName];
  }
  return {
    get: /* @__PURE__ */ __name(async (key) => memoryStore3.get(key) || null, "get"),
    put: /* @__PURE__ */ __name(async (key, value, opts) => {
...
var AI_MODELS = [
  ...
  // 1-3 Hours: Novice (Experimental)
  {
    id: "nous-hermes-2-mixtral",
    name: "Nous Hermes 2 Mixtral",
    provider: "Nous Research",
    description: "Experimental fine-tune. Good for creative writing.",
    parameters: "8x7B",
    type: "Experimental",
    minTier: "novice",
  ...
  // 3-10 Hours: Apprentice (Wild/Uncensored)
  {
    id: "dolphin-mixtral",
    name: "Dolphin Mixtral (Uncensored)",
    provider: "Cognitive Computations",
    description: "Uncensored model. Will answer almost anything.",
    parameters: "8x7B",
    type: "Uncensored",
    minTier: "apprentice",
  ...
  // 10-24 Hours: Adept (Roleplay/18+)
  {
    id: "pygmalion-mythalion",
    name: "Pygmalion Mythalion",
    provider: "PygmalionAI",
    description: "Optimized for roleplay and character interaction.",
    parameters: "13B",
    type: "Roleplay",
    minTier: "adept",
    openrouterId: "pygmalionai/mythalion-13b",
    hasRealApi: true,
  ...
  },
  {
    ...
    provider: "Gryphe",
    description: "State-of-the-art roleplay model. Highly creative.",
    parameters: "13B",
    type: "Roleplay",
    minTier: "adept",
  ...
  // 24+ Hours: Master (Broken Protocol)
  {
    id: "goliath-120b",
    name: "Goliath 120B (Broken Protocol)",
    provider: "Alpindale",
    description: "Massive, unhinged model. Use effectively with no safety rails.",
    parameters: "120B",
    type: "Unrestricted",
    minTier: "master",
  ...
  // Missing Models that Frontend Has
  {
    id: "llama-3.3-70b",
    name: "LLaMA 3.3",
    provider: "Meta AI",
    description: "Latest generation large language model.",
    parameters: "70B",
    type: "General Purpose",
    minTier: "explorer",
    // Make it accessible
  ...
  },
  {
    ...
    name: "Gemini 1.5 Pro",
    provider: "Google DeepMind",
    description: "Reasoning model with huge context window.",
    parameters: "Unknown",
    type: "General Purpose",
    minTier: "novice",
  ...
  },
  {
    ...
    name: "Qwen 3",
    provider: "Alibaba Cloud",
    description: "Multimodal model with strong performance.",
    parameters: "72B",
    type: "Multimodal",
    minTier: "explorer",
    // Make it accessible
...
function canAccessModel(user, modelId) {
  const model = AI_MODELS.find((m) => m.id === modelId);
  if (!model) return false;
  if (user.isDeveloper) return true;
  if (user.tier === "developer") return true;
  const firstConnected = new Date(user.firstConnected || (/* @__PURE__ */ new Date()).toISOString());
  const now = /* @__PURE__ */ new Date();
  const hoursConnected = (now.getTime() - firstConnected.getTime()) / (1e3 * 60 * 60);
  const requiredTier = TIERS[model.minTier];
  if (!requiredTier) return false;
  return hoursConnected >= requiredTier.minHours;
}
__name(canAccessModel, "canAccessModel");
async function hashRequest(data) {
...
__name(hashRequest, "hashRequest");
async function logModelAccess(kv, walletAddress, modelId, requestData, responseStatus) {
  const logEntry = {
    timestamp: (/* @__PURE__ */ new Date()).toISOString(),
    walletAddress,
    modelId,
    requestHash: await hashRequest(requestData),
    responseStatus
  };
  const logKey = `access_log:${Date.now()}:${Math.random().toString(36).substring(2, 9)}`;
  await kv.put(logKey, JSON.stringify(logEntry), { expirationTtl: 2592e3 });
  const userStatsKey = `model_stats:${walletAddress}`;
  const existingStats = await kv.get(userStatsKey);
  const stats = existingStats ? JSON.parse(existingStats) : {};
  stats[modelId] = stats[modelId] || { count: 0, firstAccess: logEntry.timestamp };
  stats[modelId].count++;
  stats[modelId].lastAccess = logEntry.timestamp;
  await kv.put(userStatsKey, JSON.stringify(stats));
}
__name(logModelAccess, "logModelAccess");
function getCurrentTier(user) {
  if (user.isDeveloper || user.tier === "developer") return "developer";
  const firstConnected = new Date(user.firstConnected || (/* @__PURE__ */ new Date()).toISOString());
  const now = /* @__PURE__ */ new Date();
  const hoursConnected = (now.getTime() - firstConnected.getTime()) / (1e3 * 60 * 60);
  if (hoursConnected >= 24) return "master";
  if (hoursConnected >= 10) return "adept";
  if (hoursConnected >= 3) return "apprentice";
  if (hoursConnected >= 1) return "novice";
  return "explorer";
}
__name(getCurrentTier, "getCurrentTier");
async function checkRateLimit(kv, walletAddress, user) {
  const tier = getCurrentTier(user);
  const rateLimitKey = `rate_limit:${walletAddress}:${Math.floor(Date.now() / 6e4)}`;
  const current = await kv.get(rateLimitKey);
  const count = current ? parseInt(current) : 0;
  const maxRequests = Math.floor(TIERS[tier].rateLimitPerMinute * TEST_MODE.rateLimitMultiplier);
  if (count >= maxRequests) {
    return { allowed: false, remaining: 0, resetAt: Math.ceil(Date.now() / 6e4) * 6e4 };
  }
  await kv.put(rateLimitKey, (count + 1).toString(), { expirationTtl: 120 });
  return { allowed: true, remaining: maxRequests - count - 1, resetAt: Math.ceil(Date.now() / 6e4) * 6e4 };
}
__name(checkRateLimit, "checkRateLimit");
async function checkMonthlyUsage(kv, walletAddress, user) {
  const tier = getCurrentTier(user);
  const monthKey = (/* @__PURE__ */ new Date()).toISOString().substring(0, 7);
  const usageKey = `monthly_usage:${walletAddress}:${monthKey}`;
  const current = await kv.get(usageKey);
  const used = current ? parseInt(current) : 0;
  const maxRequests = TIERS[tier].maxRequestsPerMonth;
  if (TEST_MODE.enabled || user.isDeveloper) {
    return { allowed: true, used, remaining: 999999 };
  }
  if (used >= maxRequests) {
    return { allowed: false, used, remaining: 0 };
  }
  return { allowed: true, used, remaining: maxRequests - used };
}
__name(checkMonthlyUsage, "checkMonthlyUsage");
async function callAIProvider(env, modelId, prompt, parameters) {
  const openAIKey = env.OPENAI_API_KEY;
  const openRouterKey = env.OPENROUTER_API_KEY;
  const modelDef = AI_MODELS.find((m) => m.id === modelId);
  const errors = [];
  const makeRequest = /* @__PURE__ */ __name(async (url, key, modelName, headers = {}) => {
    let requestBody = {
      model: modelName,
      messages: [{ role: "user", content: prompt }],
      temperature: parameters.temperature ?? 0.7,
      max_tokens: parameters.maxTokens ?? 1e3,
    ...
    if (!response.ok) {
      ...
      throw new Error(`Status ${response.status}: ${errorText.substring(0, 200)}`);
    }
    return await response.json();
  }, "makeRequest");
  if (openRouterKey && modelDef?.openrouterId) {
    try {
      const data = await makeRequest(
        "https://openrouter.ai/api/v1/chat/completions",
        openRouterKey,
        modelDef.openrouterId,
        {
      ...
      console.error("OpenRouter attempt failed:", error);
  ...
  if (openAIKey) {
    const modelMap = {
      ...
      "qwen-3-72b": "gpt-4o-mini",
      "deepseek-v3": "gpt-4o",
      "mistral-large": "gpt-4o"
    };
    const openAIModelName = modelMap[modelId] || "gpt-4o-mini";
    try {
      const data = await makeRequest(
...
*[System Note: OpenRouter failed (${errors[0]}). Response provided by OpenAI fallback.]*`;
      }
      return {
        response: content,
        usage: data.usage
      ...
      console.error("OpenAI attempt failed:", error);
...
function generateMockResponse(model, prompt, parameters) {
  const reason = model.flags.isUnethical ? "unethical" : model.flags.isBanned ? "banned" : "unavailable";
  if (reason === "unavailable") {
    return `[MOCK RESPONSE - Service Unavailable]

Model: ${model.name}
Prompt: "${prompt.substring(0, 100)}..."

The real API for this model is currently unavailable or encountered an error. This is a fallback response.

Please check your API keys or try again later.`;
  }
  return `[MOCK RESPONSE - Research Mode]

Model: ${model.name}
Prompt: "${prompt.substring(0, 100)}..."

This model (${model.id}) is flagged as ${reason} and is only available for research purposes. Real API integration is disabled for safety.

In a production research environment with proper oversight, this would connect to the actual model endpoint.`;
}
__name(generateMockResponse, "generateMockResponse");
var onRequestPost4 = /* @__PURE__ */ __name(async (context) => {
  try {
    return await handleModelPost(context);
  } catch (globalError) {
    return new Response(
      JSON.stringify({ error: "Unexpected server error", details: String(globalError) }),
      { status: 500, headers: { "Content-Type": "application/json" } }
    );
  }
}, "onRequestPost");
async function handleModelPost(context) {
  const startTime = Date.now();
  try {
    const authHeader = context.request.headers.get("Authorization");
    if (!authHeader || authHeader === "Bearer anonymous") {
      return new Response(
        JSON.stringify({ error: "Authentication required - Please sign in with your email on the Platform page" }),
        { status: 401, headers: { "Content-Type": "application/json" } }
      );
    }
    const userEmail = authHeader.replace("Bearer ", "").toLowerCase();
    const usageKv = getStore3(context.env, "MODEL_USAGE_KV");
    const rateKv = getStore3(context.env, "RATE_LIMIT_KV");
    const usersKv = getStore3(context.env, "USERS_KV");
    const userData = await usersKv.get(`email:${userEmail}`);
    let user = userData ? JSON.parse(userData) : { tier: "explorer", firstConnected: (/* @__PURE__ */ new Date()).toISOString() };
    if (!user.firstConnected) {
      user.firstConnected = (/* @__PURE__ */ new Date()).toISOString();
      await usersKv.put(`email:${userEmail}`, JSON.stringify(user));
    }
    const userTier = user.tier || "explorer";
    const { modelId, prompt, parameters = {} } = await context.request.json();
    if (!modelId || !prompt) {
      return new Response(
        JSON.stringify({ error: "modelId and prompt required" }),
        { status: 400, headers: { "Content-Type": "application/json" } }
      );
    }
    if (!canAccessModel(user, modelId)) {
      await logModelAccess(usageKv, userEmail, modelId, { prompt }, 403);
      const requiredModel = AI_MODELS.find((m) => m.id === modelId);
      const currentTier2 = getCurrentTier(user);
      return new Response(
        JSON.stringify({
          error: "Access denied",
          message: `Your current tier (${currentTier2}) needs more time to unlock this model.`,
          requiredTier: requiredModel?.minTier,
          upgradeAvailable: true
        }),
        { status: 403, headers: { "Content-Type": "application/json" } }
      );
    }
    const rateLimitStatus = await checkRateLimit(rateKv, userEmail, user);
    if (!rateLimitStatus.allowed) {
      const currentTier2 = getCurrentTier(user);
      return new Response(
        JSON.stringify({
          error: "Rate limit exceeded",
          message: `Limit: ${TIERS[currentTier2].rateLimitPerMinute} requests/minute`,
          resetAt: rateLimitStatus.resetAt
        }),
        { status: 429, headers: { "Content-Type": "application/json" } }
      );
    }
    const usageStatus = await checkMonthlyUsage(usageKv, userEmail, user);
    if (!usageStatus.allowed) {
      const currentTier2 = getCurrentTier(user);
      return new Response(
        JSON.stringify({
          error: "Monthly quota exceeded",
          message: `You've used ${usageStatus.used} of ${TIERS[currentTier2].maxRequestsPerMonth} requests this month.`,
          used: usageStatus.used,
          limit: TIERS[currentTier2].maxRequestsPerMonth
        }),
        { status: 429, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    if (model.costPer1kTokens > 0 && !user.isDeveloper) {
      const userTokens = user.tokens || 0;
      if (userTokens <= 0) {
        return new Response(
          JSON.stringify({
            error: "Insufficient tokens",
            message: "This model requires tokens. Please purchase a token pack or upgrade to Developer Mode.",
            upgradeAvailable: true
          }),
    ...
    let apiUsage = null;
    let estimatedCost = 0;
    if (context.env.OPENAI_API_KEY || context.env.OPENROUTER_API_KEY) {
      try {
        const result = await callAIProvider(context.env, modelId, prompt, parameters);
        aiResponse = result.response;
        apiUsage = result.usage;
        if (apiUsage) {
          const totalTokens = (apiUsage.prompt_tokens || 0) + (apiUsage.completion_tokens || 0);
          estimatedCost = totalTokens / 1e3 * model.costPer1kTokens;
        } else {
          const estimatedTokens = (prompt.length + aiResponse.length) / 4;
          estimatedCost = estimatedTokens / 1e3 * model.costPer1kTokens;
        }
      } catch (error) {
        ...
        return new Response(
          JSON.stringify({
            ...
            note: "The server rejected the request. Please verify your API keys and model access."
          }),
          { status: 502, headers: { "Content-Type": "application/json" } }
        );
      }
    } else {
      aiResponse = generateMockResponse(model, prompt, parameters);
      const estimatedTokens = (prompt.length + aiResponse.length) / 4;
      estimatedCost = estimatedTokens / 1e3 * model.costPer1kTokens;
    }
    if (estimatedCost > 0 && !user.isDeveloper) {
      user.tokens = (user.tokens || 0) - estimatedCost;
      await usersKv.put(`email:${userEmail}`, JSON.stringify(user));
    }
    const monthKey = (/* @__PURE__ */ new Date()).toISOString().substring(0, 7);
    await usageKv.put(
      `monthly_usage:${userEmail}:${monthKey}`,
      (usageStatus.used + 1).toString(),
      { expirationTtl: 2592e3 }
    );
    await logModelAccess(usageKv, userEmail, modelId, { prompt }, 200);
    const warnings = [];
    if (model.flags.isBanned) warnings.push("This model is banned from general use. Your access is logged for research transparency.");
    if (model.flags.isUnethical) warnings.push("\u26A0\uFE0F RESEARCH USE ONLY: This model generates potentially harmful content. All outputs must be handled responsibly.");
    if (model.flags.isUncensored) warnings.push("This model has no safety filters. Outputs may be unpredictable or inappropriate.");
    if (!model.openrouterId) warnings.push("This model uses simulated responses for research purposes.");
    ...
    return new Response(
      JSON.stringify({
        ...
        usage: {
          ...
          limit: TIERS[currentTier].maxRequestsPerMonth
        },
        tokens: {
          used: estimatedCost,
        ...
        realApi: !!context.env.OPENAI_API_KEY || !!context.env.OPENROUTER_API_KEY
      }),
      {
        status: 200,
        headers: {
...
__name(handleModelPost, "handleModelPost");
  ...
  try {
    ...
    if (!authHeader || authHeader === "Bearer anonymous") {
      return new Response(
        JSON.stringify({
          tier: "explorer",
          usage: { used: 0, limit: TIERS["explorer"].maxRequestsPerMonth, remaining: TIERS["explorer"].maxRequestsPerMonth },
          testMode: TEST_MODE.enabled
        }),
        { status: 200, headers: { "Content-Type": "application/json" } }
      );
    ...
    const userTier = user.tier || "explorer";
    const monthKey = (/* @__PURE__ */ new Date()).toISOString().substring(0, 7);
    const usageKey = `monthly_usage:${userEmail}:${monthKey}`;
    const used = await usageKv.get(usageKey);
    const modelStats = await usageKv.get(`model_stats:${userEmail}`);
    return new Response(
      JSON.stringify({
        tier: userTier,
        usage: {
          used: used ? parseInt(used) : 0,
          limit: TIERS[userTier].maxRequestsPerMonth,
          remaining: TIERS[userTier].maxRequestsPerMonth - (used ? parseInt(used) : 0)
        },
        modelBreakdown: modelStats ? JSON.parse(modelStats) : {},
        tierInfo: {
          name: TIERS[userTier].name,
          rateLimit: TIERS[userTier].rateLimitPerMinute
        },
...
function getStore4(env) {
  ...
  return {
    get: /* @__PURE__ */ __name(async (key) => memoryStore4.get(key) || null, "get"),
    put: /* @__PURE__ */ __name(async (key, value) => {
...
var onRequestPost5 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const store = getStore4(context.env);
...
var onRequestPost6 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    if (!openRouterKey) {
      return new Response(JSON.stringify({ error: "No OPENROUTER_API_KEY" }), {
        status: 500,
        headers: { "Content-Type": "application/json" }
      });
    }
    ...
    if (!response.ok) {
      ...
      return new Response(JSON.stringify({ error: `OpenRouter failed: ${response.status}`, details: errText.substring(0, 300) }), {
    ...
    return new Response(JSON.stringify({ error: "Crash", details: error.message }), {
      status: 500,
      headers: { "Content-Type": "application/json" }
    });
  }
...
var onRequestGet6 = /* @__PURE__ */ __name(async () => {
  return new Response(JSON.stringify({ status: "test endpoint alive" }), {
    status: 200,
    headers: { "Content-Type": "application/json" }
  });
}, "onRequestGet");
...
function getStore5(env, bindingName) {
  if (env[bindingName]) {
    return env[bindingName];
  }
  return {
    get: /* @__PURE__ */ __name(async (key) => memoryStore5.get(key) || null, "get"),
    put: /* @__PURE__ */ __name(async (key, value) => {
...
__name(canAccessModel2, "canAccessModel");
...
var onRequestGet7 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    if (authHeader && authHeader !== "Bearer anonymous") {
      const store = getStore5(context.env, "USERS_KV");
      const userEmail = authHeader.replace("Bearer ", "").toLowerCase();
  ...
  try {
    ...
    switch (action) {
      ...
      case "getStatus":
        ...
        return new Response(
          JSON.stringify({
            currentTier: user.tier || "explorer",
            canUpgrade: TEST_MODE2.autoVerify,
            testMode: TEST_MODE2.enabled,
...
function getStore6(env) {
  if (env.USERS_KV) {
    return env.USERS_KV;
  }
  return {
    get: /* @__PURE__ */ __name(async (key) => memoryStore6.get(key) || null, "get"),
    put: /* @__PURE__ */ __name(async (key, value) => {
...
var onRequestPost8 = /* @__PURE__ */ __name(async (context) => {
  try {
    ...
    const store = getStore6(context.env);
    const normalizedAddress = address.toLowerCase();
    if (action === "connect") {
...
// ../.wrangler/tmp/pages-8tHJBR/functionsRoutes-0.23442802604454804.mjs
var routes = [
  {
